...
\cellcolor{cyan}    
\procedureItem{Inform \textbf{\#hephaestus-testing} about the test using the slack workflow}{}
  ...
  \procedureItem{Check weather forecast according to Meteo Swiss: 
    \begin{itemize}
      \item Temperatures within operating temperatures (-5$^\circ$C -- 35$^\circ$C)
      \item Limited wind, no hard rain and no storms (DACS must stay dry)
    \end{itemize}}{}
 
  \cellcolor{cyan} 
  \iftoggle{firing}{\procedureItem{Check that access request to airfield for all attendees has been sent out to MP (check on excel list)}{HEP Sharepoint $\rightarrow$ Testing}}{}
  
  \cellcolor{cyan}    
  \procedureItem{Check that Emergency Driver 1 and Emergency Driver 2 are NEVER working on the pressurized system at the same time (no two of TC, SO and EIC)}{}
  
  \cellcolor{cyan}    
  \procedureItem{Check that all contingency procedures are printed (sec. D)}{If changes $\rightarrow$ Print new version}
  ...
  \cellcolor{cyan}    
  \procedureItem{Charge tablets if necessary (ensure a charger cable is available)}{}
  ...
  \cellcolor{cyan}    
  \procedureItem{Check that all relevant procedures are printed out (sec. D) or that everyone has the relevant procedures downloaded on their tablet}{}
